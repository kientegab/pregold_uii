...
export class IndicateurComponent {
  ...
  indicateurListSubscription: Subscription | undefined;
   ...
   ngOnInit(): void {
      ...
      ngOnDestroy(): void {
        if (this.routeData) {
          ...
          if (this.indicateurListSubscription) {
            this.indicateurListSubscription.unsubscribe();
          }
        }
      }
      ...
      openModalCreate(): void {
        this.dialogService.open(CreerModifierIndicateurComponent,
          ...
          this.showMessage({ severity: 'success', summary: 'indicateur creer avec succès' });
          }
        });
      }
      ...
      openModalDetail(indicateur:IIndicateur): void {
        this.dialogService.open(DetailIndicateurComponent,
          {
            header: 'Details de indicateur',
            width: '60%',
            contentStyle: { overflow: 'auto' },
            baseZIndex: 10000,
            maximizable: true,
      ...
      onDelete(indicateur: IIndicateur) {
        this.confirmationService.confirm({
          message: 'Etes-vous sur de vouloir supprimer ce indicateur?',
          accept: () => {
      ...
      delete(selection: any) {
        ...
        this.indicateurService.delete(selection.idIndicateur).subscribe(() => {
